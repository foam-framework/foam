...
KeyboardShortcutController.prototype.shouldIgnoreKeyEventsForTarget_ =
    function(event) {
...
var DOM = {
  setClass: function(e, className, opt_enabled) {
    ...
    e.className =
        oldClassName.replace(' ' + className, '').replace(className, '');
  ...
  initHTML: function() {
    ...
    if (this.callbacks_) {
      ...
      for (var i = 0; i < this.callbacks_.length; i++) {
        ...
        var e = $(elementId);
        if (! e) {
          console.log('Error Missing element for id: ' + elementId +
              ' on event ' + event);
        } else {
...
var AbstractView2 = FOAM({
  ...
  methods: {
    ...
    nextID: function() {
      return 'view2_' +
          (arguments.callee._nextId = (arguments.callee._nextId || 0) + 1);
    },
...
var Canvas = Model.create({
  ...
  methods: {
    ...
    toHTML: function() {
      return '<canvas id="' + this.getID() + '" width="' + this.width +
...
var circleModel = Model.create({
  ...
  methods: {
    ...
    paint3d: function() {
      ...
      var radgrad = canvas.createRadialGradient(this.x + this.r / 6,
          this.y + this.r / 6, this.r / 3, this.x + 2, this.y, this.r);
...
var Box = FOAM({
  ...
  methods: {
    paint: function() {
      ...
      var grad = c.createLinearGradient(this.x, this.y, this.x + this.width,
          this.y + this.height);
    ...
    },
    {
      ...
      postSet: function(newValue, oldValue) {
        if (this.mode === 'read-write') {
          ...
          Events.relate(newValue, this.domValue, this.valueToText,
              this.textToValue);
        } else {
  ...
  methods: {
    toHTML: function() {
      return (this.mode === 'read-write') ?
          '<input id="' + this.getID() + '" type="' + this.type + '" name="' +
          this.name + '" size=' + this.displayWidth + '/>' :
    ...
    initHTML: function() {
      ...
      this.domValue = this.mode === 'read-write' ?
          DomValue.create(e, this.onKeyMode ? 'input' : undefined) :
          DomValue.create(e, undefined, 'innerHTML');
...
var DateTimeFieldView = FOAM({
  ...
  methods: {
    ...
    toHTML: function() {
      return (this.mode === 'read-write') ?
          '<input id="' + this.getID() + '" name="' + this.name + '" size=' +
          this.displayWidth + '/>' :
          '<span id="' + this.getID() + '" name="' + this.name + '"></span>';
    }
  }
 ...
 */
  ...
  methods: {
    toHTML: function() {
      return '<select id="' + this.getID() + '" name="' + this.name +
    ...
    updateHTML: function() {
      ...
      for (var i = 0; i < this.choices.length; i++) {
        ...
        if (Array.isArray(choice)) {
          // var encodedValue = choice[0].replace(/\"/g, '&quot;')
          //     .replace(/</g, '&lt;').replace(/>/g, '&gt;');
          // var encodedValue = choice[0].replace(/"/g, '*')
          //     .replace(/</g, '*').replace(/>/g, '*');
          if (this.value && choice[0] === this.value.get()[0]) {
            out.push(' selected');
          }
          out.push(' value="');
          out.push(i + '">');
    ...
    initHTML: function() {
      ...
      Events.dynamic(function() { this.choices; }.bind(this),
    ...
    updateHTML: function() {
      ...
      for (var i = 0; i < this.choices.length; i++) {
        ...
        /*
           ...
           out.push(this.value && choice[0] == this.value.get() ?
        ...
        */
          ...
          out.push(choice.toString());
          out.push('"');
          var callback = (function(value, choice) {
            return function() { value.set(choice); }
          })(this.value, choice);
...
var BooleanView = FOAM({
  ...
  methods: {
    toHTML: function() {
      ...
      str += '<input type="checkbox" id="' + this.getID() + '" name="' +
          this.name + '" />';
...
var TextAreaView = FOAM({
  ...
  methods: {
    ...
    toHTML: function() {
      return '<textarea id="' + this.getID() + '" rows=' + this.rows +
    ...
    initHTML: function() {
      this.domValue = DomValue.create(this.$, this.onKeyMode ? 'input' :
          'change', 'value');

      // Events.follow(this.model, this.domValue);
      // Events.relate(this.value, this.domValue, this.valueToText,
      //     this.textToValue);
...
var FunctionView = FOAM({
  ...
  methods: {
    ...
    toHTML: function() {
      return '<pre style="color:red">' + this.errorView.toHTML() + '</pre>' +
          this.SUPER();
    },
...
var DetailView2 = {
  ...
  toHTML: function() {
    ...
    return '<div id="' + this.getID() +
        '" class="detailView" name="form">dv2</div>';
  },
...
var TableView2 = FOAM({
  ...
  methods: {
    ...
    layout: function() {
      var parent = window.getComputedStyle(
...
style = window.getComputedStyle(this.$.children[0]);
    ...
    repaint: function() {
      ...
      (this.sortOrder ? this.dao.orderBy(this.sortOrder) : this.dao)
          .limit(this.rows).select({
    ...
    tableToHTML: function() {
      ...
      if (this.callbacks_) {
        // console.log('Warning: TableView2.tableToHTML called twice without' +
        //     ' initHTML');
        delete this['callbacks_'];
        this.children = [];
      }
      ...
      for (var i = 0; i < this.properties.length; i++) {
        ...
        if (this.sortOrder === prop) {
          arrow = ' <span class="indicator">&#9650;</span>';
            ...
            this.sortOrder.isDESC && this.sortOrder.c === prop) {
          arrow = ' <span class="indicator">&#9660;</span>';
        }
...
var ActionButton = {
  ...
  toHTML: function() {
    ...
    var out = [];
    out.push('<button class="actionButton actionButton-' + this.action.name +
        '" id="' + this.getID() + '">');
  ...
  methods: {
    ...
    openAsMenu: function(document) {
      ...
      div.onmouseout = function(e) {
        if (e.toElement.parentNode != div &&
            e.toElement.parentNode.parentNode != div) {
          div.parentNode.removeChild(div);
        }
      };
    ...
    toHTML: function(opt_menuMode) {
      ...
      for (var i = 0; i < this.actions.length; i++) {
        ...
        str += this.preButton(button) + button.toHTML() +
...
/* TODO:
   ...
   Buttons should be own View with enabled = true/false and own listeners,
   don't use <button> directly
 */
var ActionBorder = {
...
var ProgressView = FOAM({
  ...
  methods: {
    ...
    toHTML: function() {
      return '<progress value="25" id="' + this.getID() +
          '" max="100" >25</progress>';
    },

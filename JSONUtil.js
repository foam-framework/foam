      ...
      if (obj instanceof Object) {
         for (var key in obj) {
            if (key != 'model_' && key != 'prototype_') {
              obj[key] = this.mapToObj(obj[key]);
            }
         }

         if (! GLOBAL[obj.model_]) {
            // console.log("Missing class: ", obj);

            //    if ( opt_modelLoader ) return opt_modelLoader(obj.model_);

            return obj;
         }
         return GLOBAL[obj.model_].create(obj);
      }
   ...
   compressed: {
      ...
      stringify: function(obj) {
         return Iuppiter.Base64.encode(
             Iuppiter.compress(JSONUtil.compact.stringify(obj), true));
      }
   }

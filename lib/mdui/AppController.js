...
//      this.X = this.X.sub();
  ...
  templates: [
    ...
    <%
      this.addInitializer(function() {
        if ( self.filterChoices ) {
          ...
          v.index$.addListener(function() {
            self.qView.el.placeholder = "Search " + v.views[v.index].label.toLowerCase();
          });
        }
      ...
      this.on('touchstart', function(){ console.log('blurring'); self.qView.el.blur(); }, this.data.filteredDAOView.id);
      this.on('click', function(){ console.log('focusing'); self.qView.el.focus(); }, this.qView.id);
    %>
  */},
    function CSS() {/*
...
MODEL({
  ...
  methods: {
    ...
    toInnerHTML: function() {
      ...
      var view = this.filteredDAOView = this.X.CanvasScrollView.create({

...
MODEL({
  ...
  methods: {
    ...
    select: function(sink, options) {
      ...
      options.limit && urlParams.push('maxResults=' + options.limit);
...
MODEL({
  ...
  properties: [
    ...
    { model_: 'BooleanProperty', name: 'isDraft', defaultValue: false },
    ...
    toRaw: function() {
      ...
      function doPart(p) {
        ...
        if ( p.body && p.body.size > 0 ) {
          var decoder = Base64Decoder.create(IncrementalUtf8.create(), p.body.size);
      ...
      if ( ! obj.raw )  {
        doPart(this.payload);
        obj.raw = Base64Encoder.create({ urlSafe: true }).encode(new Uint8Array(stringtoutf8(buffer)));
      }
      return obj;
    }
  }
...
MODEL({
  ...
  properties: [
    { name: 'model', defaultValueFn: function() { return FOAMGMailMessage; }, hidden: true },
    ...
    { name: 'draftDao', factory: function() { return GMailRestDAO.create({ model: GMailDraft }); }, hidden: true },
    { name: 'localDao', factory: function() { return NullDAO.create({ model: GMailMessage }); }, hidden: true },

    { name: 'historyDao', factory: function() { return GMailRestDAO.create({ model: GMailHistory }); }, hidden: true },
    { name: 'lastHistoryId' },
        ...
        aif(function() { return obj.labelIds.indexOf('DRAFT') != -1; },
            aseq(
              ...
              function(ret, success) {
                if ( ! success ) {
                  fc.errorEvt('Could not find matching draft for ' + obj.id);
                }
                ret();
              })),
        function(ret) {
    ...
    sendDraft_: function(ret, obj, sink) {
      ...
      aseq(
        ...
        function(ret, response, xhr) {
          ...
          sink && sink.put && sink.put(obj);
          if ( self.daoListeners_.length > 0 ) {
            self.notify_('remove', [obj]);
    ...
    put: function(obj, sink) {
      ...
      aif(
        ...
        aseq(
          ...
          aif(obj.isSent, function(ret) { self.sendDraft_(ret, obj, sink); })),
        aseq(
          function(ret) {
...
MODEL({
  ...
  methods: {
    aToB: function(obj) {
      var msg =  FOAMGMailMesage.create({
        id: obj.id,
        labelIds: obj.labelIds,
      ...
      headers.push({ name: 'Cc', value: obj.cc });
      headers.push({ name: 'Bcc', value: obj.bcc });
      headers.push({ name: 'Reply-To', value: obj.replyTo });
      ...
      var payload = {
        ...
        body: {
          ...
          value: Base64Encoder.create({ urlSafe: true }).encode(obj.body)
        }
      };
    ...
    bToA: function(obj) {
      ...
      var decode = function (str) {
        var decoder = Base64Decoder.create(IncrementalUtf8.create());
        decoder.put(str);
        decoder.eof();

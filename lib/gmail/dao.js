  ...
  methods: {
    ...
    find: function(key, sink) {
      ...
      this.SUPER(key, {
        ...
        error: function(_, status) {
          if ( status === 404 ) {
            sink && sink.put && sink.put(this.model.create({ id: key, deleted: true }));
            return;
          }
          ...
          if ( items && items.length ) {
            ...
            items.forEach(function(i) {
              i.messages.forEach(function(j) {
                pars.push(function(ret) { self.find(j.id, ret); });
              });
            });
          }
          aseq(

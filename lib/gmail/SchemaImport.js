...
var SCHEMA_URL = 'https://www.googleapis.com/discovery/v1/apis/gmail/v1/rest';
...
var GMailSchema = {
 ...
 "ownerName": "Google",
 "icons": {
  "x16": "http://www.google.com/images/icons/product/googlemail-16.png",
  "x32": "http://www.google.com/images/icons/product/googlemail-32.png"
 },
...
function generateModel(schema, prefix, package) {
  var mps = [];
  prefix = prefix || '';
...
var exports = [
   GMailSchema.schemas.Draft,
   GMailSchema.schemas.History,
   GMailSchema.schemas.Label,
   GMailSchema.schemas.Message,
   GMailSchema.schemas.Thread
];
...
var dao = X.node.dao.JSModelFileDAO.create();

var pars = [];
exports.map(function(m, i) {
  var model = generateModel(m, 'GMail', 'com.google.mail');
  model.plural = plurals[i];
  pars.push(
    function(ret) {
      dao.put(model, { put: function() {
        ret();
      }, error: function() {
        console.log("ERROR: Failed to write ", model.id);
        ret();
      }});
    });
});

apar.apply(null, pars)(function() { process.exit(0); });

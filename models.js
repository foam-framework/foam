   ...
   methods: {
      connect: function(e) {
         e.addEventListener('mousemove',
             EventService.merged(this.onMouseMove, 1));
      }
   },
...
//       this.canvasView = Canvas.create(this);
         this.canvasView = Canvas.create(
             {width: this.width + 1, height: this.height + 2});
         if (this.backgroundColor) {
           this.canvasView.backgroundColor = this.backgroundColor;
         }
...
var CView = FOAM({
   ...
   methods: {
      toHTML: function() {
         ...
         // If being added to HTML directly, then needs to create own Canvas
         // as parent.
   ...
   methods: {
      ...
      paintBarData: function(canvas, x, y, xs, w, h, maxValue)
      {
         ...
         for (var i = 0; i < this.data.length; i++)
         {
            ...
            canvas.fillRect(
                x1, y1, w / this.data.length + 1.5, d * h / maxValue);
         }
      },
      ...
      paint: function()
      {
         ...
         if (this.style == 'Line') {
           this.paintLineData(canvas, x, y, xs, w, h, maxValue);
         } else if (this.style == 'Bar') {
           this.paintBarData(canvas, x, y, xs, w, h, maxValue);
         } else if (this.style == 'Point') {
           this.paintPointData(canvas, x, y, xs, w, h, maxValue);
         }
         ...
         for (var i = this.xAxisInterval; i <= this.data.length;
             i += this.xAxisInterval) {
            var x2 = this.toX(i);
            ...
            if (this.gridColor) {
               ...
               canvas.fillRect(x2 + 1.5,
                   this.toY(0, 1) - 2 * xs, 0.5, -this.height);
               canvas.restore();
            }
         ...
         for (var i = this.yAxisInterval; i <= maxValue;
             i += this.yAxisInterval) {
      ...
      toX: function(val) {
         ...
         return this.x + this.axisSize +
             (val == 0 ? 0 : w * val / (this.data.length - 1));
      },
...
//           if (this.view && this.view.model_ &&
//               this.view.model_.getProperty('dao')) this.view.dao = dao;
           }
       },
       {
          name: 'view',
...
//       Events.link(this.model, this.view.model);
      ...
      toHTML: function() {
         ...
         str.push('<div style="width:100%;margin-bottom:5px;">' +
             '<div class="altViewButtons">');
         for (var i = 0; i < this.views.length; i++) {
            var view = this.views[i];
            var listener = function(altView, view) { return function(e) {
               altView.view = view;

               // This is a bit hackish, each element should listen on a
               // 'selected' property and update themselves
...
//          str.push('<a href="#top" id="' + this.registerCallback(
//              'click', listener) + '">' + view.label + '</a>');
            str.push('<a class="buttonify" id="' +
                this.registerCallback('click', listener) + '">' +
                view.label + '</a>');
...
//       str.push(this.view.toHTML());
      ...
      initHTML: function() {
         ...
         DOM.setClass($(this.buttons_[this.buttons_.length - 1][0]),
             'capsule_right');
      }

...
var DAOController = FOAM({
   ...
   methods: {
      init: function() {
         ...
         var dao = this.dao;
         this.tableView =
             TableView2.create({ model: model, dao: dao, rows: 30 });
...
//       this.scrollBorder.initHTML(); // could this just be added to children?
         ...
         this.dao = this.dao;
         this.tableView.unsubscribe(
             this.tableView.DOUBLE_CLICK, this.onDoubleClick);
         this.tableView.subscribe(
             this.tableView.DOUBLE_CLICK, this.onDoubleClick);
...
//             (this.stackView || stack).setPreview(view);
               ...
               (this.stackView || stack).setPreview(
                   SummaryView.create(this.tableView.selection));
//             (this.stackView || stack).setPreview(
//                 DetailView.create(obj.model_, new SimpleValue(obj)));
            }
            else
            {

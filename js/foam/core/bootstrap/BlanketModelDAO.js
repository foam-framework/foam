...
MODEL({
  ...
  name: 'BlanketModelDAO',

  documentation: 'For use with Blanket.js code coverage library',
    ...
    find: function(key, sink) {
      ...
      aseq(
        ...
        function(ret, data) {
          ...
          // Blanket instrumentation of the code
          blanket.instrument({
            inputFile: data,
            inputFileName: key,
          }, function(iData) {
            var oldCb = GLOBAL.__DATACALLBACK;
            GLOBAL.__DATACALLBACK = function(iData, latch) {
              var work = [anop];
              var obj = JSONUtil.mapToObj(this.X, iData, undefined, work);

              if ( ! obj ) throw new Error('Failed to decode iData: ' + iData);

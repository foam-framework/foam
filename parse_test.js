...
//console.log('***********',
//    calc.parse(calc.expr, ErrorReportingPS.create(stringPS('1+2+3 '))).value);
...
//  0xba, 0x01, 0x0b, 0x10, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
//  0xff, 0x01, 0xf2, 0xff, 0x1f, 0x0e, 0x12, 0x05, 0x68, 0x65, 0x6c, 0x6c,
//  0x6f, 0x12, 0x05, 0x77, 0x6f, 0x72, 0x6c, 0x64]);
//var testdata = new Uint8Array([
//  0xba, 0x01, 0x0b, 0x10, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
//  0xff, 0x01]);
...
var sample = '\n\n\n message Person // asdfasdf \n\n\n' +
    '{ /* required asdfasdf \n\n */ optional int32 id = 1; ' +
    'required string name = 2; optional CompoundEmail email = 3; }\n\n\n' +
    'message CompoundEmail { optional string name = 1;' +
...
var instance = protomodels[0].create({
  id: 12,
  name: 'adam',
  email: protomodels[1].create({ name: 'adamvy', domain: 'google.com' })
});

...
var EventService = {
    ...
    /** Create a "one-time" listener which unsubscribes itself after its first
     * invocation. **/
    ...
    /**
     * Merge all notifications occuring in the specified time window into a
     * single notification. Only the last notification is delivered.
     * @param opt_delay time in milliseconds of time-window, defaults to 16ms,
     *     which is the smallest delay that humans aren't able to perceive.
     **/
    ...
    publish: function(topic) {
       return this.subs_ ? this.pub_(
           this.subs_, 0, topic,
           this.appendArguments([this, topic], arguments, 1)) : 0;
    },
...
/** Extend EventService with support for dealing with property-change
 * notification. **/
var PropertyChangeSupport = {
   ...
   /** @arg property the name of the property to listen to or 'null' to listen
    * to all properties. **/
...
var Events = {
    ...
    /** Have the dstValue listen to changes in the srcValue and update its
     ...
     * Initial value is copied from value1 to model2. **/
    ...
    /** Unlink the values of two models by having them no longer follow each
     * other. **/
...
var Movement = {
   ...
   avg: function(f1, f2) {
     return function(x) { return (f1(x) + f2(x)) / 2; };
   },
     ...
     function compileParallel(op, rest) {
       ...
       return function() {
         for (var i = 0; i < op.length; i++)
           if (isSimple(op[i]))
             Movement.animate(op[i][0], op[i][1], op[i][2],
                 Movement.seq(op[i][3], join))();
           else
...
window.requestAnimationFrame &&
window.requestAnimationFrame(contextize(function() {

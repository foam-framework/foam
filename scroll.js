...
var ScrollCView = FOAM({
  ...
  listeners: {
    mouseDown: function(e) {
      ...
      e.target.ownerDocument.defaultView.addEventListener(
          'mouseup', this.mouseUp, true);
      e.target.ownerDocument.defaultView.addEventListener(
          'mousemove', this.mouseMove, true);
      e.target.ownerDocument.defaultView.addEventListener(
          'touchstart', this.touchstart, true);
      this.mouseMove(e);
    },
    mouseUp: function(e) {
      e.preventDefault();
      e.target.ownerDocument.defaultView.removeEventListener(
          'mousemove', this.mouseMove, true);
      e.target.ownerDocument.defaultView.removeEventListener(
          'mouseup', this.mouseUp, true);
    ...
    mouseMove: function(e) {
      ...
      this.value = Math.max(0, Math.min(this.size - this.extent,
          Math.round((y - this.y) / (this.height - 4) * this.size)));
    },
    ...
    touchMove: function(e) {
      ...
      e.preventDefault();
      this.value = Math.max(0, Math.min(this.size - this.extent,
          Math.round(this.startValue +
          (y - this.startY) / (this.height - 4) * this.size)));
    },
  ...
  properties: [
    ...
    },
    {
      ...
      valueFactory: function() {
        var sb = ScrollCView.create(
            {height: 1800, width: 20, x: 2, y: 2, extent: 10});
    ...
    },
    {
      ...
      postSet: function(newValue, oldValue) {
        ...
        this.dao.select(COUNT())(function(c) {
          ...
          self.scrollbar.value = Math.max(0,
              Math.min(self.scrollbar.value,
                     self.scrollbar.size - self.scrollbar.extent));
          if (self.dao) {
            self.view.dao = self.dao.skip(self.scrollbar.value);
          }
        });
        /*
      ...
      view.$.onmousewheel = function(e) {
        if (e.wheelDeltaY > 0 && scrollbar.value) {
          scrollbar.value--;
            ...
            scrollbar.value < scrollbar.size - scrollbar.extent) {
          scrollbar.value++;
        }
      };

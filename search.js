...
var GroupBySearchView = FOAM({
  ...
  properties: [
    {
      ...
      valueFactory: function() {
        return ChoiceView.create(
            {size: this.size, cssClass: 'foamSearchChoiceView'}); }
    },
    {
      name: 'width',
      type: 'int',
  ...
  methods: {
    ...
    initHTML: function() {
      ...
      // Events.dynamic(function() { this.view.value; },
      // console.log.bind(console));
      Events.dynamic(function() { this.dao; }, this.updateDAO);
      this.propertyValue('filter').addListener(this.updateDAO);
      /*
       this.propertyValue('filter').addListener(
       (function(a,b,oldValue,newValue) {
         this.updateDAO();
       }).bind(this));
      */
      ...
      [
       {
         ...
         code: function() {
           ...
           this.dao.where(this.filter).select(
               GROUP_BY(this.property, COUNT()))(function(groups) {
             var options = [];
             for (var key in groups.groups) {
               var count = ('(' + groups.groups[key] + ')').intern();
               var subKey = key.substring(0, self.width - count.length - 3);
               var cleanKey = subKey.replace(/</g, '&lt;')
                   .replace(/>/g, '&gt;').replace(/"/g, '&quot;');
               options.push([key, cleanKey +
                 (Array(self.width - subKey.length - count.length)
                   .join('&nbsp;')).intern() + count]);
             }
             options.sort();
  ...
  methods: {
    toHTML: function() {
      return '<div class="foamSearchView">' +
          ...
          '<div id=' + this.registerCallback('click', this.clear) +
          ' style="text-align:right;width:100%;float:right;' +
          'margin-bottom:20px;" class="searchTitle">' +
          '<font size=-1><u>Clear</u></font></div>';
    },
    initHTML: function() {

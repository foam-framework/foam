...
var Action = FOAM({
  ...
  properties: [
    ...
    },
    {
      ...
      type: 'String',
      defaultValue: true,
      help: 'Property indicating whether the label should be rendered along ' +
          'side the icon'
    },
    {
      name: 'action',
  ...
  templates: [
    {
      ...
      template: '<%= this.returnType || "void" %> <%= this.name %>(' +
          ...
          'var arg = this.args[i]; %>' +
          '<%= arg.javaSource() %><% if ( i < this.args.length-1 ) ' +
          'out(", ");%><% } %>)'
    },
    {
      model_: 'Template',
      ...
      template:
          ...
          '<% } %>' +
          ' */\n' +
          '<%= arguments[1] %>.prototype.<%= this.name %> = ' +
          'goog.abstractMethod;'
    },
    {
      model_: 'Template',
...
var Interface = FOAM({
  ...
  templates: [
    ...
    },
    {
      ...
      description: 'Web IDL Source',
      template:
          'interface <%= this.name %> <% if (this.extends.length) { %>: ' +
          '<%= this.extends[0] %> <% } %>{\n' +
          '<% for ( var i = 0 ; i < this.methods.length ; i++ ) {' +
          'var meth = this.methods[i]; %>' +
          '  <%= meth.webIdl() %>;\n' +
